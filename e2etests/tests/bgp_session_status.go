...
var _ = ginkgo.Describe("BGPSessionState", func() {
	...
	ginkgo.Context("BGP and BFD", func() {
		...
		ginkgo.DescribeTable("Each node manages its statuses", func(p params) {
			...
			expectedStatusVRF := p.vrf //TODO!
			if p.vrf == "" {
				expectedStatusVRF = "default"
			}

			ginkgo.By("Verifying the status resources are created with bgp Active or Connect")
			for _, n := range neighbors {
				Eventually(func() error {
					return validateStatusFor(nodesNames, n.Address, expectedStatusVRF, false, sets.New("Active", "Connect"), sets.New("N/A"))
				}, 1*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
			}

			Consistently(func() error {
				for _, n := range neighbors {
					err := validateStatusFor(nodesNames, n.Address, expectedStatusVRF, false, sets.New("Active", "Connect"), sets.New("N/A"))
					if err != nil {
						return err
					}
				}
			...
			for _, n := range neighbors {
				Eventually(func() error {
					return validateStatusFor(nodesNames, n.Address, expectedStatusVRF, false, sets.New("Established"), sets.New("N/A"))
				}, 1*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
			}
			...
			for _, n := range neighbors {
				Eventually(func() error {
					return validateStatusFor(nodesNames, n.Address, expectedStatusVRF, false, sets.New("Established"), sets.New("down"))
				}, 1*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
			}
			...
			for _, n := range neighbors {
				Eventually(func() error {
					return validateStatusFor(nodesNames, n.Address, expectedStatusVRF, false, sets.New("Established"), sets.New("up"))
				}, 1*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
			}

			Consistently(func() error {
				for _, n := range neighbors {
					err := validateStatusFor(nodesNames, n.Address, expectedStatusVRF, false, sets.New("Established"), sets.New("up"))
					if err != nil {
						return err
					}
				}
			...
			for _, n := range neighbors {
				Eventually(func() error {
					err := validateStatusFor([]string{nodesNames[0]}, n.Address, expectedStatusVRF, true, sets.New[string](), sets.New[string]())
					if err != nil {
						return err
					}
					return validateStatusFor(nodesNames[1:], n.Address, expectedStatusVRF, false, sets.New("Established"), sets.New("up"))
				}, 1*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
			}

			// drop config from 1 container, validate it updates
			ginkgo.By("blabla time")
			...
			for _, n := range neighbors {
				Eventually(func() error {
					if frr0Addresses.Has(n.Address) {
						return validateStatusFor(nodesNames[1:], n.Address, expectedStatusVRF, false, sets.New("Active", "Connect"), sets.New("down"))
					}
					return validateStatusFor(nodesNames[1:], n.Address, expectedStatusVRF, false, sets.New("Established"), sets.New("up"))
				}, 1*time.Minute, 2*time.Second).ShouldNot(HaveOccurred())
			}
		},
			ginkgo.Entry("AAAA IPV4", params{
